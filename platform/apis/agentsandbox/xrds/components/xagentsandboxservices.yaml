...
metadata:
  name: xagentsandboxservices.platform.bizmatters.io
spec:
  group: platform.bizmatters.io
  names:
    kind: XAgentSandboxService
    plural: xagentsandboxservices
  claimNames:
    kind: AgentSandboxService
    plural: agentsandboxservices
  versions:
    - name: v1alpha1
      served: true
      referenceable: true
      schema:
        openAPIV3Schema:
          ...
          properties:
            spec:
              type: object
              description: "AgentSandboxService specification for deploying sandboxed agent execution environments with hybrid persistence"
              required:
                - image
                - nats
              properties:
                ...
                command:
                  type: array
                  description: "Optional command override for the main container"
                  items:
                    type: string

                args:
                  type: array
                  description: "Optional arguments for the main container command"
                  items:
                    type: string
                ...
                # AgentSandboxService-specific fields for workspace persistence
                storageGB:
                  type: integer
                  description: "Workspace storage size in GB for persistent volumes"
                  minimum: 1
                  maximum: 1000
                  default: 10
                  example: 20